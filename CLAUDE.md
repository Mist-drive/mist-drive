...
make dev-desktop      # wails dev with webkit2_41 tag
...
- **`internal/config/`** — All config from env vars. Required: `JWT_SECRET`, `ADMIN_PASSWORD`, `S3_ENDPOINT`, `S3_ACCESS_KEY`, `S3_SECRET_KEY`. Defaults: `JWT_TTL=24h` (tokens without a refresh session), `ACCESS_TTL=15m` + `REFRESH_TTL=720h` (web/desktop refresh sessions, `REFRESH_TTL=0` disables), `DATA_DIR=./data`, `LOG_PATH=./logs/app.log`, `MAX_ZIP_BYTES=20GiB`, `ZIP_STREAM_TIMEOUT=30m` (Go duration; max wall-time for one folder-zip stream — the download ticket TTL only gates the start, this bounds the whole transfer). Optional SMTP block: `SMTP_HOST/PORT/USER/PASSWORD/FROM/TLS` — all empty by default (email disabled).
...
- **Refresh storms (learned the hard way)**: `files-changed` is coalesced server-side (`events.CoalesceWindow`, 750ms trailing) AND both clients' `refresh()` are single-flight (one in flight, one queued — see web `Files/index.tsx`, desktop `Files.tsx`). Without the client guard, a mass upload of small files stacks slow list requests until the browser's ~6-conn-per-origin pool exhausts and every request "pends" — looks exactly like a dead API but isn't. Related: web `serverLost` no-ops while the WS is OPEN (an open push channel is proof of life; prevents false logouts under client-side congestion).
...
- **Data dirs**: `DATA_DIR` defaults to `./data` (relative to where the binary runs). In dev (`make dev-api`), `api/.env` sets `DATA_DIR=../data/api` so data lands in `data/api/` at the repo root, not inside `api/`. Docker uses the default `./data` which resolves to `/app/data` inside the container (WORKDIR `/app`). `LOG_PATH` follows the same pattern.
- **`dev-api` env**: `DATA_DIR` and `LOG_PATH` are written into `api/.env` on first run by the Makefile and sourced via `set -a && . ./.env`. The `.air.toml` `[env]` section is unreliable across air versions — env vars go in `.env` instead.
...
- **Real client IP**: `fiber.Config{ProxyHeader: fiber.HeaderXForwardedFor}` in `main.go` so `c.IP()` reads from Traefik's `X-Forwarded-For` header. Now load-bearing (not cosmetic): the per-IP login throttle keys off it. **Hardened in the stack** (`cy17-single-node-services-stack/docker-compose.yml`): both Traefik entrypoints set `forwardedHeaders.trustedIPs` to loopback + RFC1918, so Traefik (the edge) overwrites client-supplied XFF for public clients instead of appending → first value can't be spoofed. **✅ Verified in prod (2026-06-11)**: a spoofed `X-Forwarded-For: 1.2.3.4` on `POST /auth/login` logged the real client IP, not the spoof. To re-verify after Traefik changes: Traefik must be *recreated* (static config), then `curl -H "X-Forwarded-For: 1.2.3.4" https://drive.<domain>/auth/login -d '{"login":"nope","password":"nope"}' -H 'Content-Type: application/json'` and confirm the `auth: unknown user` line shows the real IP. The logger fans out to BOTH stdout (text → `docker compose logs mist-drive`/Dozzle) and the lumberjack-rotated JSON file at `LOG_PATH` (`logs/mist-drive/app.log`) — either has the line. Note `docker compose logs` is project-scoped: run it from the deployed stack dir, not the standalone `mist-drive` repo. If a CDN/LB is ever added in front, append its egress CIDRs to both `trustedIPs` lists. In local dev XFF is absent → `clientIP()` helper falls back to `c.Context().RemoteAddr()` TCP address, then `"unknown"`.
...
- **Email notifications**: new-IP login → `SendNewIP` to the affected user's `Email` field; wrong-password × 3 → `SendFailedLogin` to admin's `Email` field (paced by the per-login failure count returned from `loginFail`, `count%3`). Both fire in goroutines (non-blocking). `make dev-api` starts Mailpit on port 1025 (UI at http://localhost:8025) and seeds `api/.env` with `SMTP_HOST=localhost SMTP_PORT=1025 SMTP_TLS=none`.
